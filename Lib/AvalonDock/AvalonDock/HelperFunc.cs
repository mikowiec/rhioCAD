...
namespace AvalonDock
{
    internal static class HelperFunc
    {
        ...
        public static bool AreClose(double v1, double v2)
        {
            ...
            var num = ((Math.Abs(v1) + Math.Abs(v2)) + 10.0)*2.2204460492503131E-16;
            var num2 = v1 - v2;
        ...
        public static T FindAnotherLogicalChildContained<T>(this DependencyObject obj, UIElement childToExclude)
        ...
        private static DockablePane FindChildDockablePane(UIElement parent)
        {
            if (parent is ResizingPanel)
            {
                foreach (UIElement childObject in ((ResizingPanel) parent).Children)
                {
                    var foundPane = FindChildDockablePane(childObject);
                    if (foundPane != null)
                        return foundPane;
                }
            }
